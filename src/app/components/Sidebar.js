...
const Sidebar = () => {
  ...
  const activeBar="sm:rounded-l-lg bg-sky-200 shadow-lg";
  const toggleSidebar = () => {
    setIsOpen(!isOpen);
  };
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 z-10 bg-white transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative md:w-80 md:flex ${isOpen ? 'top-24' : 'top-0'} shadow-xl`}>
        <nav className="flex flex-col pl-4 pt-4 space-y-4 text-black w-80">
          <Link href="/" legacyBehavior >
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
          ...
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/categories' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
          ...
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/admins' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
              <FontAwesomeIcon icon={faPerson} className="w-5 h-5 mr-2 text-red-600" />
              <span className="md:block">Admin</span>
            </a>
          </Link>
          <Link href="/orders" legacyBehavior>
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/orders' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
          ...
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/shipping' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
              <FontAwesomeIcon icon={faShippingFast} className="w-5 h-5 mr-2 text-green-600" />
              <span className="md:block">Shipping</span>
            </a>
          </Link>
          <Link href="/settings" legacyBehavior>
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/settings' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
              <FontAwesomeIcon icon={faCog} className="w-5 h-5 mr-2 text-green-600" />
              <span className="md:block">Settings</span>
            </a>
          </Link>
          <Link href="/" legacyBehavior>
          <a className={`flex items-center p-3 gap-3 text-black text-xl ${pathname === '/logout' ? 'sm:rounded-l-lg bg-sky-200 shadow-lg' : ''}`}>
